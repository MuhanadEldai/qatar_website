document.addEventListener('DOMContentLoaded', function() {
    ...
    function submitBooking() {
        ...
        const time = activeSlot ? activeSlot.querySelector('.time-range').textContent : 'Morning';
    ...
    function loadGallery() {
        ...
        const galleryItems = [{ title: 'Dune Bashing', desc: 'Thrilling safari' }, { title: 'Sunset View', desc: 'Desert magic' }];
        galleryGrid.innerHTML = galleryItems.map(item => `
            <div class="gallery-item">
                <img src="https://placehold.co/600x400/FFD700/333?text=${item.title}" alt="${item.title}">
                <div class="gallery-caption"><h6>${item.title}</h6></div>
            </div>`).join('');
    }
    ...
    function updatePaginationUI() {
        const dots = document.querySelector('.viewpager-dots');
        if (!dots) return;
        dots.innerHTML = Array.from({length: totalSlides}, (_, i) => `<button class="viewpager-dot ${i === currentSlide ? 'active' : ''}" onclick="goToSlide(${i})"></button>`).join('');
    }
    ...
    function updateLoadMoreButton() { /* Your existing btn logic */ }
