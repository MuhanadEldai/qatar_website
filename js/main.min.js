...
function submitBooking() {
    ...
    // Capture all fields accurately
    const name = form.querySelector('input[placeholder*="Name"]').value;
    const phone = form.querySelector('input[type="tel"]').value;
    const email = form.querySelector('input[type="email"]').value;
    const date = form.querySelector('input[type="date"]').value;
    const location = form.querySelector('input[placeholder*="Location"], input[placeholder*="Hotel"]').value;
    ...
    const timeLabel = activeSlot ? activeSlot.querySelector('.time-label').textContent : 'Morning';
    const timeRange = activeSlot ? activeSlot.querySelector('.time-range').textContent : '8:30-12:30';
    ...
    const message = `📋 *NEW BOOKING REQUEST* 📋%0A%0A` +
        ...
        `📧 *Email:* ${email}%0A` +
        `📅 *Date:* ${date}%0A` +
        `⏰ *Time:* ${timeLabel} (${timeRange})%0A` +
        `📍 *Pickup:* ${location}%0A` +
        `👥 *Adults:* ${adults}%0A` +
        `👶 *Children:* ${children}%0A` +
        `💭 *Requests:* ${requests || 'None'}%0A%0A` +
    ...
    // Show Success Modal (Ensure you have a div with id="successModal")
    ...
    const galleryItems = [
        { title: 'Falconry Experience', desc: 'Traditional Qatari culture', img: 'falcon.jpg' },
        ...
        { title: 'Sunset Magic', desc: 'Golden hour in the desert', img: 'sunset.jpg' }
    ];
  ...
  function updatePaginationUI() {
    ...
    function updateBookingSummary(t, v) { /* Your existing summary logic */ }
    function showNoReviews() { /* Your existing no-reviews logic */ }
});
